...
class static_shader(sp.ShaderProgram):
    ...
    def get_all_uniform_locations(self):
        ...
        for x in range(0, self.MAX_LIGHTS):
            ...
            self.location_light_position[x] = super(static_shader, self).get_uniform_location(
                "light_position[" + str(x) + "]")
            self.location_light_color[x] = super(static_shader, self).get_uniform_location(
                "light_color[" + str(x) + "]")
            self.location_attenuation[x] = super(static_shader, self).get_uniform_location(
                "attenuation[" + str(x) + "]")
