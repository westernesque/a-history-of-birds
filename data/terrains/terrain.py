...
class terrain():
    ...
    def get_terrain_height(self, world_x, world_z):
        ...
        if x_coordinate <= (1.0 - z_coordinate):
            b_coord = m.Maths().barycentric_coordinates((0, self.heights[grid_x][grid_z], 0),
                                                        (1, self.heights[grid_x + 1][grid_z], 0),
                                                        (0, self.heights[grid_x][grid_z + 1], 1),
                                                        (x_coordinate, z_coordinate))
        else:
            b_coord = m.Maths().barycentric_coordinates((1, self.heights[grid_x + 1][grid_z + 1], 0),
                                                        (1, self.heights[grid_x][grid_z + 1], 1),
                                                        (0, self.heights[grid_x][grid_z + 1], 1),
                                                        (x_coordinate, z_coordinate))
        # print b_coord
        return b_coord
