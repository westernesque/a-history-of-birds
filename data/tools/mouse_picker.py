...
class mouse_picker():
	...
	def __init__(self, camera, projection_matrix, display, terrain):
		...
		self.count = 0
		self.view_matrix = m.maths().create_view_matrix(self.camera) # is a numpy.array
	...
	def update(self):
		...
		print self.current_ray
		if self.intersection_in_range(0, self.ray_range, self.current_ray) == True:
			self.terrain_point = self.binary_search(self.count, 0, self.ray_range, self.current_ray)
		else:
			self.terrain_point = None
	def calcutate_mouse_ray(self):
		mouse_x = float(pygame.mouse.get_pos()[0])
		mouse_y = float(pygame.mouse.get_pos()[1])
	...
	def get_normalized_device_coordinates(self, mouse_x, mouse_y):
		...
		return (x, -y) #possibly (x, -y)
		

		
	def get_point_on_ray(self, ray, distance):
		camera_position = self.camera.get_position()
		start = numpy.array([camera_position[0], camera_position[1], camera_position[2]], dtype = "float32")
		scaled_ray = numpy.array([ray[0] * distance, ray[1] * distance, ray[2] * distance], dtype = "float32")
